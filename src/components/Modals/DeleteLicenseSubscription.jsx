...
export const DeleteLicenseSubscriptionModal = ({id})  =>{
  const [open, setOpen] = React.useState(false);
  const queryClient = useQueryClient();
  const { data, mutate, isLoading, isError, isSuccess, error} = useMutation({
    mutationFn: async (id) => {
      return await AssetQuery.AssetManagement.deleteLicenseSubscription(id);
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries("licensesubscriptions");
    }
  });
  ...
  return (
    <div>
      <Tooltip title="Delete LicenseSubscription">
        <IconButton 
        color="error"
        size="small"
        aria-label=""
